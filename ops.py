...
class TaskBuilder:
    ...
    def build(context, setting, objects, active_obj):
        mode = setting.bake_mode; tasks = []
        if mode == 'SINGLE_OBJECT':
            for obj in objects:
                n = TaskBuilder._get_name(setting, obj)
                tasks.append(BakeTask(objects=[obj], materials=[ms.material for ms in obj.material_slots if ms.material], active_obj=obj, base_name=n, folder_name=n))
            ...
            tasks.append(BakeTask(objects=objects, materials=list(all_mats), active_obj=active_obj, base_name=n, folder_name=n))
        elif mode == 'SELECT_ACTIVE':
            if not active_obj: return []
            n = TaskBuilder._get_name(setting, active_obj)
            tasks.append(BakeTask(objects=objects, materials=[ms.material for ms in active_obj.material_slots if ms.material], active_obj=active_obj, base_name=n, folder_name=n))
        elif mode == 'SPLIT_MATERIAL':
            for obj in objects:
                for ms in obj.material_slots:
                    if not ms.material: continue
                    n = TaskBuilder._get_name(setting, obj, ms.material)
                    tasks.append(BakeTask(objects=[obj], materials=[ms.material], active_obj=obj, base_name=n, folder_name=n))
        return tasks
...
class BAKETOOL_OT_BakeOperator(bpy.types.Operator):
    ...
    @classmethod
    def poll(cls, context): return context.scene.BakeJobs and context.scene.BakeJobs.jobs
    def execute(self, context):
        try:
            for job in context.scene.BakeJobs.jobs: self.process_job(context, job)
            if context.scene.BakeJobs.jobs[0].setting.save_and_quit: bpy.ops.wm.save_mainfile(exit=True)
        except Exception as e:
            import traceback; logger.error(traceback.format_exc())
            self.report({'ERROR'}, f"Bake Error: {str(e)}"); return {'CANCELLED'}
        self.report({'INFO'}, "Baking Finished"); return {'FINISHED'}

    def process_job(self, context, job):
        s = job.setting
        objs = [o.bakeobject for o in s.bake_objects if o.bakeobject]
        if not objs and context.selected_objects: objs = [o for o in context.selected_objects if o.type == 'MESH']
        if not objs: return
        act = s.active_object or context.active_object
        tasks = TaskBuilder.build(context, s, objs, act)
        chans = self._get_enabled_channels(s)
        if s.use_custom_map: chans.extend(self._get_custom_channels(job))
        if not chans: return
        ...
        with SceneSettingsContext('scene', scene_sets), SceneSettingsContext('image', img_sets), SceneSettingsContext('cm', cm_sets):
            for task in tasks: self.process_task(context, s, task, chans)
            if s.export_model: self._export_results(s, tasks)

    def process_task(self, context, setting, task, channels):
        bpy.ops.object.select_all(action='DESELECT')
        for o in task.objects: o.select_set(True)
        if task.active_obj: context.view_layer.objects.active = task.active_obj
        handler = NodeGraphHandler(task.materials); task_images = {}
        try:
            for c in channels:
                img = self.process_channel(context, setting, task, c, handler, task_images)
                if img and c['id'] != 'CUSTOM': task_images[c['channel_prop'].id] = img
    ...
    def process_channel(self, context, setting, task, c, handler, task_images):
        ...
        if cid in ('ID_mat', 'ID_ele', 'ID_UVI', 'ID_seam', 'position', 'UV', 'wireframe'):
            mesh_type = None; attr_name = None
            if cid.startswith('ID_') or cid == 'ID_seam':
                mesh_type = 'ID'
                id_key = cid.split('_')[1].upper() if '_' in cid else 'ELEMENT'
                # [修改] 传递用户指定的优化参数
                attr_name = setup_mesh_attribute(task.active_obj, id_key, setting.id_start_color, setting.id_use_annealing)
        ...
        elif cid == 'CUSTOM': self._bake_custom(setting, task, c, handler, img, task_images)
        else:
            handler.setup_for_pass(bake_pass=c['bake_pass'], socket_name=c['node_socket'], image=img)
            self._call_bake_api(setting, c)
        if setting.save_out: save_image(image=img, path=setting.save_path, folder=setting.create_new_folder, folder_name=task.folder_name, file_format=setting.save_format, reload=setting.reload)
    ...
    def _bake_custom(self, s, task, c, handler, target, sources):
        ...
        for mat in task.materials:
            ...
            t_node = nodes.new('ShaderNodeTexImage'); t_node.image = target; nodes.active = t_node; handler.temp_nodes[mat].append(t_node)
            emi = nodes.new('ShaderNodeEmission'); handler.temp_nodes[mat].append(emi)
            if prop.bw:
                out = self._build_source(nodes, links, prop, 'bw', sources, handler.temp_nodes[mat]); links.new(out, emi.inputs[0])
            else:
                ...
                for i, char in enumerate(['r','g','b']):
                    out = self._build_source(nodes, links, prop, char, sources, handler.temp_nodes[mat]); links.new(out, comb.inputs[i])
                links.new(comb.outputs[0], emi.inputs[0])
            out_node = handler._get_output_node(tree)
            if out_node:
                sock = out_node.inputs[0]
                if mat not in handler.history: handler.history[mat] = {'sock': sock, 'src': sock.links[0].from_socket if sock.is_linked else None}
                links.new(emi.outputs[0], sock)
        bpy.ops.object.bake(type='EMIT', margin=s.margin, use_clear=s.clearimage)

    def _build_source(self, nodes, links, prop, char, sources, temps):
        pre = char + "_"; use_map = getattr(prop, pre + "usemap")
        if not use_map:
            n = nodes.new('ShaderNodeValue'); n.outputs[0].default_value = getattr(prop, char) if char != 'bw' else 0.0
            temps.append(n); return n.outputs[0]
        src_id = getattr(prop, pre + "source"); src_img = sources.get(src_id)
        if not src_img:
            n = nodes.new('ShaderNodeValue'); temps.append(n); return n.outputs[0]
        tex = nodes.new('ShaderNodeTexImage'); tex.image = src_img; temps.append(tex)
        try: tex.image.colorspace_settings.name = 'Non-Color'
        except: pass
        out = tex.outputs[0]
        if getattr(prop, pre + "sepcol"):
            sep = nodes.new('ShaderNodeSeparateColor' if bpy.app.version >= (3,3,0) else 'ShaderNodeSeparateRGB')
            temps.append(sep); links.new(out, sep.inputs[0]); idx = {'R':0,'G':1,'B':2,'A':2}.get(getattr(prop, pre + "colchan"), 0); out = sep.outputs[idx]
        if getattr(prop, pre + "invert"):
            inv = nodes.new('ShaderNodeInvert'); temps.append(inv); links.new(out, inv.inputs[1]); inv.inputs[0].default_value = 1.0; out = inv.outputs[0]
        return out
    ...
    def _get_enabled_channels(self, s):
        ...
        for c in s.channels:
            ...
            res.append({'id': c.id, 'name': c.name, 'channel_prop': c, 'bake_pass': info.get('bake_pass', 'EMIT'), 'node_socket': info.get('node_socket'), 'prefix': c.prefix, 'suffix': c.suffix, 'custom_cs': c.custom_cs})
        return res

    def _get_custom_channels(self, job):
        return [{'id': 'CUSTOM', 'name': c.name, 'channel_prop': c, 'bake_pass': 'EMIT', 'node_socket': None, 'prefix': c.prefix, 'suffix': c.suffix, 'custom_cs': c.color_space} for c in job.Custombakechannels]
    ...
    def _call_bake_api(self, s, c):
        ...
        if c['bake_pass'] == 'NORMAL': params['normal_space'] = 'OBJECT' if c['channel_prop'].normal_obj else 'TANGENT'
        if s.bake_mode == 'SELECT_ACTIVE': params.update({'use_selected_to_active': True, 'use_cage': bool(s.cage_object), 'cage_object': s.cage_object.name if s.cage_object else "", 'cage_extrusion': s.extrusion, 'max_ray_distance': s.ray_distance})
        pf = set(); cp = c['channel_prop']
        if c['id'] == 'diff':
    ...
    def _write_result(self, context, image, type_name, obj_name):
        ...
        res.image = image; res.channel_type = type_name; res.object_name = obj_name
...
class BAKETOOL_OT_GenericChannelOperator(bpy.types.Operator):
    bl_idname = "bake.generic_channel_op"; bl_label = "Op"; bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        bj = context.scene.BakeJobs; job = bj.jobs[bj.job_index] if bj.jobs else None
        props_map = {"jobs_channel": (bj.jobs, "job_index"), "job_custom_channel": (job.Custombakechannels if job else None, "Custombakechannels_index"), "bake_objects": (job.setting.bake_objects if job else None, "active_object_index")}
        if self.target not in props_map: return {'CANCELLED'}
        coll, idx_name = props_map[self.target]
        if coll is None: return {'CANCELLED'}
        idx = getattr(bj if self.target=="jobs_channel" else job if self.target=="job_custom_channel" else job.setting, idx_name)
        if self.action_type == 'ADD':
            item = coll.add(); item.name = f"New {len(coll)}"
            setattr(bj if self.target=="jobs_channel" else job if self.target=="job_custom_channel" else job.setting, idx_name, len(coll)-1)
            if self.target == "jobs_channel": bpy.ops.bake.reset_channels()
        elif self.action_type == 'DELETE' and len(coll) > 0:
            coll.remove(idx); setattr(bj if self.target=="jobs_channel" else job if self.target=="job_custom_channel" else job.setting, idx_name, min(idx, len(coll)-1))
        elif self.action_type == 'CLEAR':
            coll.clear(); setattr(bj if self.target=="jobs_channel" else job if self.target=="job_custom_channel" else job.setting, idx_name, -1)
        elif self.action_type in ('UP', 'DOWN') and len(coll) > 1:
            new_idx = idx - 1 if self.action_type == 'UP' else idx + 1
            if 0 <= new_idx < len(coll): coll.move(idx, new_idx); setattr(bj if self.target=="jobs_channel" else job if self.target=="job_custom_channel" else job.setting, idx_name, new_idx)
        return {'FINISHED'}
...
class BAKETOOL_OT_ResetChannels(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        defs = []
        if s.bake_type == 'BSDF': defs.extend(CHANNEL_DEFINITIONS['BSDF_4' if bpy.app.version >= (4,0,0) else 'BSDF_3'])
        elif s.bake_type in CHANNEL_DEFINITIONS: defs.extend(CHANNEL_DEFINITIONS[s.bake_type])
        if s.use_special_map: defs.extend(CHANNEL_DEFINITIONS['MESH'])
        ...
        for d in defs:
            if d['id'] not in ex_ids:
                new = s.channels.add(); new.id = d['id']; new.name = d['name']
                if 'defaults' in d:
        ...
        for t_idx, d_id in enumerate(d_ids):
            ...
            if c_idx != -1 and c_idx != t_idx: s.channels.move(c_idx, t_idx)
        return {'FINISHED'}

class BAKETOOL_OT_SetSaveLocal(bpy.types.Operator):
    bl_idname="bake.set_save_local"; bl_label="Local"
...
class BAKETOOL_OT_RecordObjects(bpy.types.Operator):
    bl_idname="bake.record_objects"; bl_label="Record"
    objecttype: props.IntProperty(default=0); allobjects: props.BoolProperty(default=False)
    def invoke(self,context,event): self.allobjects=event.shift; return self.execute(context)
    def execute(self,context):
        if not context.scene.BakeJobs.jobs: return {'CANCELLED'}
        s = context.scene.BakeJobs.jobs[context.scene.BakeJobs.job_index].setting
        if self.objecttype==0:
            s.bake_objects.clear()
            for o in [o for o in context.selected_objects if o.type=='MESH' and (not self.allobjects or o!=context.active_object)]:
...
class BAKETOOL_OT_BakeSelectedNode(bpy.types.Operator):
    ...
    @classmethod
    def poll(cls, context): return context.active_object and context.active_node and context.active_object.active_material
    def execute(self, context):
        bj = context.scene.BakeJobs; mat = context.active_object.active_material; nt = mat.node_tree
        ...
        with SceneSettingsContext('scene', sets), SceneSettingsContext('image', imgs), SceneSettingsContext('cm', {'view_transform': 'Standard'}):
            for node in [n for n in nt.nodes if n.select]:
                ...
                nt.links.new(sock, out.inputs[0]); bpy.ops.object.bake(type='EMIT', margin=bj.node_bake_margin)
                if bj.node_bake_save_outside: save_image(img, bj.node_bake_save_path, file_format=bj.node_bake_save_format, reload=bj.node_bake_reload)
                else: img.pack()
...
class BAKETOOL_OT_DeleteResult(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if idx >= 0:
            ...
            context.scene.baked_image_results.remove(idx); context.scene.baked_image_results_index = max(0, idx-1)
        return {'FINISHED'}
...
class BAKETOOL_OT_DeleteAllResults(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        context.scene.baked_image_results.clear(); context.scene.baked_image_results_index = -1
        return {'FINISHED'}

class BAKETOOL_OT_ExportResult(bpy.types.Operator):
    bl_idname = "baketool.export_result"; bl_label = "Export"
...
class BAKETOOL_OT_ExportAllResults(bpy.types.Operator):
    bl_idname = "baketool.export_all_results"; bl_label = "Export All"
