...
> [!IMPORTANT]
> **状态更新（2026-09-11）**：本报告为 2026-06-15 时点的历史审查记录，其中的"无阻断性缺陷""建议发布"结论**已被后续独立审计推翻**——针对性复现发现了数据保护、输出正确性、预设往返、后台执行与发布包验证等方面的问题。完整诊断见 [`RELEASE_AUDIT_2026-09-11.md`](RELEASE_AUDIT_2026-09-11.md)，修复与复验结果见该报告 §14 及 `CHANGELOG.md` 2026-09-11 条目。本文件仅作历史留档，**不得作为当前发布结论引用**。
...
> 本节为独立外部审计结论，**修正**上文第 1 节"无阻断性缺陷"的记录：原审计遗漏了打包、i18n locale 与边缘通道三个盲区，共发现 4 个阻断级问题，均已在本轮修复并复验。

### 7.1 已修复的阻断级问题
| ID | 问题 | 后果（修复前） | 修复 |
|---|---|---|---|
| B-01 | manifest `id`（bakenexus）≠ ZIP 目录名（baketool） | 4.2+ 扩展安装必然失败 | id 统一为 `baketool` |
| B-02 | 翻译使用 `zh_CN`，4.2+ 要求 `zh_HANS` | 中文界面全部静默回退英文 | 全量替换 + 运行时双键兼容 |
| B-03 | 5 个 Mesh 通道无引擎实现 | 用户烘出全黑贴图 | 从通道列表移除，列入 v1.1 |
| B-04 | 发布包缺 `dev_tools/` | 随包 Safety Audit 必然报错 | 打包脚本收录 |

### 7.2 复验结果
- 静态交叉验证：通道三层一致性、manifest/bl_info/目录名一致、i18n 键覆盖、打包 import 闭包——全部通过。
- 真实套件（本机 5 版本全量）：3.3.21 / 3.6.23（154P+4S）、4.2.14 / 4.5.3 / 5.0.1（158/158），0 失败 0 错误。
- 发布 ZIP：`dist/baketool-1.0.0.zip`，67 文件，根目录 == id，无泄漏。

### 7.3 遗留事项（不阻断发布）
- Normal 通道 X/Y/Z 轴向分量属性存在但引擎未读取（UI 未暴露，无用户可见影响），v1.1 决定实装或删除。
- 崩溃状态文件已按 PID 隔离，但跨实例"仅恢复自己会话"的 UI 归属仍按最新文件处理，可在 v1.1 结合 blend 文件名细化。
