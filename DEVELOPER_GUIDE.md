...
3.  **Core Layer** (`core/`): 无状态工具模块。
    *   `compat.py` (**NEW**): 版本兼容层,集中管理所有 Blender 版本差异 (3.6 - 5.0+)
    *   `execution.py` (**NEW**): 模态操作符执行逻辑,从 `ops.py` 中提取
    *   `node_manager.py`: 节点图处理,现包含 `bake_node_to_image()` 函数
    *   其他: UV, Math, Image 管理模块
...
### 2.5 Version Compatibility Layer (**v0.9.6**)
