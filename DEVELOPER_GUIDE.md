...
2.  **Engine Layer** (`core/engine.py`): The orchestrator.
    ...
    *   `BakeStepRunner`: 核心执行。
    *   **Context Management** (**v1.0.0**): `BakeContextManager` 全面迁移至 `contextlib.ExitStack`。所有的场景临时设置（渲染引擎、分辨率等）都能保证 100% 原子级回滚，无惧底层抛错。
...
- **`suite_production_workflow.py`**: 模拟真实生产流程（S2A, UDIM）。
...
- [x] **Architecture Hardening (v1.0.0)**: Robust context manager (`ExitStack`), atomic cleanup, and 100% data-driven UI.
...
- [x] **Auto-Cage 2.0**: Smart proximity-based cage generation.
